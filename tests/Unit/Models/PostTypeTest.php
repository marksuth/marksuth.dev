...
test('post type model can be instantiated', function (): void {
    $postType = new PostType;
    expect($postType)->toBeInstanceOf(PostType::class);
});

test('post type model has correct casts', function (): void {
    $postType = new PostType;
    expect($postType->getCasts())
        ->toHaveKey('meta', 'array');
});

test('post type model has correct table name', function (): void {
    $postType = new PostType;
    expect($postType->getTable())->toBe('post_types');
});

test('post type model has posts relationship', function (): void {
    $postType = new PostType;
