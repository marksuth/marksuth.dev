        ...
        <div class="third-split h-card">
            <div class="avatar-box mb-3">
                    @if ($latest_photo)
                        ...
                        <img src="{{ config('app.url') }}/images/avatar.jpg" alt="Mark Sutherland" class="avatar u-photo" height="416" width="416">
                        </button>
                    <div popover id="latest-photo">
                        <div id="latest-photo-header">
                            <small class="timestamp"><i class="fas fa-clock"></i> <time datetime="{{ $latest_photo->published_at }}">{{ $latest_photo->published_at->tz(env('APP_TIMEZONE'))->diffForHumans() }}</time></small>
                        <button type="button" popovertarget="latest-photo" popovertargetaction="hide" class="btn btn-close" title="close"><i class="fa-solid fa-times"></i></button>
                        </div>
                        <figure>
                        ...
                        <figcaption>{{ $latest_photo->title }}
                            @if ($latest_photo->meta['location'] != null)
                                <br><small><i class="fa-solid fa-location-dot"></i> {{ $latest_photo->meta['location'] }}</small>
                        @endif
                        </figcaption>
                        </figure>
                    </div>
