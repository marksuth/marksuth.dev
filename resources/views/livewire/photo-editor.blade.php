            ...
            <div class="tile tile-sm">
                <div class="media-container">
                    @if(Str::startsWith($photo->meta['mime_type'] ?? '', 'video'))
                        <video src="{{ Storage::url($photo->meta['path'] ?? '') }}" class="media" controls></video>
                    @else
                        <img src="{{ Storage::url($photo->meta['path'] ?? '') }}" alt="{{ $photo->title }}" class="media">
                    @endif
                    ...
                    <div class="modal-sidebar">
                        @if($editingPhoto)
                            @if(Str::startsWith($editingPhoto->meta['mime_type'] ?? '', 'video'))
                                <video src="{{ Storage::url($editingPhoto->meta['path'] ?? '') }}" class="media-preview" controls></video>
                            @else
                                <img src="{{ Storage::url($editingPhoto->meta['path'] ?? '') }}" alt="{{ $editingPhoto->title }}" class="media-preview">
                            @endif
                        @endif
                    </div>
